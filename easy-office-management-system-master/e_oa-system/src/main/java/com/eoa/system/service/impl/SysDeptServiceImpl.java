...
import com.eoa.common.core.domain.TreeSelect;
...
public class SysDeptServiceImpl implements ISysDeptService {
    ...
    public AjaxResult getDeptsUser() {
        ...
        List<SysDept> deptsUser = deptMapper.getDeptsUser(new String[]{deptId + ""});
        SysDept sysDept = deptsUser.get(0);
        String s = sysDept.getAncestors() + "," + deptId;
        String[] split = s.split(",");
        List<SysDept> syds = deptMapper.getDeptsUser(split);
        StringBuilder dept = new StringBuilder();
        for (SysDept syd : syds) {
            dept.append(syd.getDeptName()+" / ");
        }
        dept.deleteCharAt(dept.length()-1);
        return AjaxResult.success("获取部门成功",dept);
    }
